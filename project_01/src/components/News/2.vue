			...
			<ul>
				<li v-for='nm in newsList'>
					<a href="#">
						...
						<div class='content'>
							<p class='title'>{{nm.title}}</p>
							<div class='news-desc'>
								<p class='summary'>{{nm.source}}</p>
								<p>
									<span class='praise'>点赞数：{{nm.tcount}}</span>	
									<span class='time'>发表时间：{{nm.ptime}}</span>
								</p>
...
export default{
 ...
 //  }
	...
	created(){
		...
		.then(res=>{
			var obj=JSON.parse(JSON.stringify(res.data.data.auto));
